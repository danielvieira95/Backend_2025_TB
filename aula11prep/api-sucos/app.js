// app.cjs ou app.js (sem "type":"module")
...
const PedidoCreateSchema = z.object({
  ...
  sabor: z.string().min(1).optional(), // opcional: permitir criar pelo nome do suco
...
// POST /cadastro-suco  -> cadastra um suco
app.post("/cadastro-suco", (req, res) => {
  const parse = SucoCreateSchema.safeParse(req.body);
  if (!parse.success) {
    return res.status(400).json({ erro: parse.error.flatten() });
  }
  const { nome, preco = 0 } = parse.data;

  try {
    const stmt = db.prepare("INSERT INTO sucos (nome, preco) VALUES (?, ?)");
...
app.post("/pedidos", (req, res) => {
  ...
  // Permitir criar pelo nome do suco (sabor)
  if (!suco_id && sabor) {
  ...
  const stmt = db.prepare(`
    INSERT INTO pedidos (suco_id, quantidade, status, criado_em, atualizado_em)
    VALUES (?, ?, 'iniciado', ?, ?)
  `);
...
// PATCH /ordem-producao/:id  -> altera status (em_processamento|pronto|iniciado)
app.patch("/ordem-producao/:id", (req, res) => {
  const { id } = req.params;
  const parse = PedidoStatusSchema.safeParse(req.body);
  if (!parse.success) {
    return res.status(400).json({ erro: parse.error.flatten() });
  }
...
// GET /listar-pedidos -> por padrão, lista em andamento (status != 'pronto')
// Pode filtrar: /listar-pedidos?status=iniciado|em_processamento|pronto
